<template>
  <div class="shoplist">
    ...
    <el-table :data="shopList" border style="width: 100%">
      <el-table-column prop=""  label="" width="45" type = "selection"
      >
        <template slot-scope="scope"  >
          <el-checkbox></el-checkbox>
        </template>
      </el-table-column>
      <el-table-column prop="date" label="规格名称" width="">
        <template slot-scope="scope">
          {{scope.row.name}}
        </template>
      </el-table-column>
      <el-table-column prop='default' label="规格值" width="">
      </el-table-column>
...
export default {
  ...
  methods: {
    ...
    //请求商品列表数据
    getShopList() {
      shopApi.specification(this.page, this.limit).then(res => {
        // console.log(res);
